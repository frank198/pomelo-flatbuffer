...
class FlatBuffer
{
	...
	onUpdate(type, filePath, event)
	{
		...
		try
		{
			...
			this.logger && this.logger.debug('change proto file , type : %j, path : %j, version : %j', type, path, this.version);
		}
		catch (err)
		{
			this.logger && this.logger.warn('change proto file error! path : %j', path);
