...
class FlatBuffer
{
	...
	check(type, route)
	{
		...
		switch(type) {
			...
			case CLIENT:
				...
				flatInstance = this[cliteFlatBuffer].get(curRoute);
				break;
			default:
	...
	decode(route, message)
	{
		...
		const flatInstance = this[cliteFlatBuffer].get(curRoute);
		if (flatInstance != null)
		{
	...
	setFlatBufferData(type, filePath)
	{
		...
		else
		{
			...
			{
				this[cliteFlatBuffer].set(baseName, flatBuild);
			}
		}
	}
	...
	stop(force, cb)
	{
		for (const watcher of this[watchers].value())
		{
			if (watcher)
				watcher.close();
		}
